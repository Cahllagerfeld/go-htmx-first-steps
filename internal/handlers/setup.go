...
func (setupHander *SetupHandler) GetSetupPage(c echo.Context) error {
	...
	query, err := setupHander.githubService.GetUserRepositories(client, username, services.GithubPaginationParams{PageSize: 100, After: ""})

	if err != nil {
		return err
	}

	component := pages.SetupPage(*query)
